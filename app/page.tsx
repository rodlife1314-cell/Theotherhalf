...
import PathfinderCognitiveLayer from '@/components/PathfinderCognitiveLayer';
import { CrucibleDefenseAuditor } from '@/components/CrucibleDefenseAuditor';
import { CausalWorkflowEngine } from '@/components/CausalWorkflowEngine';
import { ParallelCognitionEngine } from '@/components/ParallelCognitionEngine';
import { LearningHelixKernel } from '@/components/LearningHelixKernel';
...
import { TrajectoryPosture } from '@/lib/pathfinder-cognition-engine';
import {
  Activity,
  Layers,
  Sparkles,
...
export default function Page() {
  ...
  const [ccvGeometryMode, setCcvGeometryMode] = useState<CCVModeGeometry>('G6');
  const [ccvPilotCommands, setCcvPilotCommands] = useState<CCVPilotCommands>({
    destinationVectorAngleDeg: 0,
    couplingAuthorityFraction: 0.92,
    stabilityEnvelope: 'tight',
  });

  const sharedCcvTelemetry = calculateCCVTelemetry(
    ccvOperatingState,
    ccvGeometryMode,
    ccvPilotCommands,
    377.0,
  ...
  const handleCommitTrajectoryToVehicle = (posture: TrajectoryPosture) => {
    if (posture === 'maintain_lock') {
      setCcvOperatingState('cruise');
      setCcvGeometryMode('G6');
      setCcvPilotCommands(prev => ({ ...prev, destinationVectorAngleDeg: 0, couplingAuthorityFraction: 0.96 }));
    } else if (posture === 'reduce_field_vector') {
      setCcvPilotCommands(prev => ({ ...prev, destinationVectorAngleDeg: 0 }));
    } else if (posture === 'rematch_impedance') {
      setCcvPilotCommands(prev => ({ ...prev, couplingAuthorityFraction: 0.94 }));
    } else if (posture === 'decouple') {
      setCcvOperatingState('release');
      setCcvPilotCommands(prev => ({ ...prev, couplingAuthorityFraction: 0.05 }));
    } else if (posture === 'standby') {
      setCcvOperatingState('acquire');
      setCcvPilotCommands(prev => ({ ...prev, couplingAuthorityFraction: 0.2 }));
    }
  };
      ...
      >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-1 text-xs font-mono border-t border-slate-800/80">
          ...
          <button
            ...
            onClick={() => setActiveTab('orbital_energy')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'orbital_energy'
                ? 'bg-blue-500/20 text-blue-300 border border-blue-500/50 shadow-sm font-bold'
          ...
          >
            ...
            <span>Orbital Energy Grid</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/30 text-blue-200 border border-blue-500/40 font-bold">
              RESONANT COUPLING
            </span>
          </button>

          {/* SIMON INFERENCE & EPISTEMIC REGISTRY (G_t → Ψ_t) */}
          <button
            id="nav-tab-simon-registry"
            onClick={() => setActiveTab('simon_registry')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'simon_registry'
                ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/50 shadow-sm font-bold'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850 border border-transparent'
            }`}
          >
            <Sparkles className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
            <span>SIMON & Registry</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-indigo-500/30 text-indigo-200 border border-indigo-500/40 font-bold">
              G_t → Ψ_t
            </span>
          </button>

          {/* GAME_ONE CODIFIED ARCHITECTURE TABS */}
          <button
            id="nav-tab-crucible"
          ...
          >
            ...
            <span>Crucible Defense</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-rose-500/30 text-rose-200 border border-rose-500/40">
              DIRTY DOZEN
            </span>
          </button>

          <button
            id="nav-tab-causal-workflow"
            onClick={() => setActiveTab('causal_workflow')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'causal_workflow'
          ...
          <button
            id="nav-tab-parallel-cognition"
            onClick={() => setActiveTab('parallel_cognition')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'parallel_cognition'
          ...
          <button
            id="nav-tab-learning-helix"
            onClick={() => setActiveTab('learning_helix')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'learning_helix'
                ? 'bg-orange-500/20 text-orange-300 border border-orange-500/50 shadow-sm font-bold'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850 border border-transparent'
            }`}
          >
            <GraduationCap className="w-3.5 h-3.5 text-orange-400 shrink-0" />
            <span>Learning Helix</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-orange-500/30 text-orange-200 border border-orange-500/40">
              PEDAGOGY
            </span>
          </button>
          ...
          >
            ...
            <span>CCV-01 Vehicle</span>
          </button>

          {/* Pathfinder Supervisory Cortex */}
          <button
            id="nav-tab-pathfinder-cortex"
            onClick={() => setActiveTab('pathfinder_cortex')}
            className={`min-h-[44px] flex items-center gap-1.5 px-3.5 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'pathfinder_cortex'
                ? 'bg-gradient-to-r from-indigo-500/25 via-purple-500/25 to-amber-500/25 text-amber-200 border border-amber-500/60 shadow-md font-bold'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850 border border-transparent'
            }`}
          >
            <Compass className="w-3.5 h-3.5 text-amber-400 shrink-0" />
            <span className="text-xs">Pathfinder Cortex</span>
          </button>
          ...
          <button
            ...
            className={`min-h-[44px] flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === 'workbench'
                ? 'bg-amber-500/20 text-amber-300 border border-amber-500/50 shadow-sm font-bold'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850 border border-transparent'
            }`}
          >
            ...
            <span className="text-xs">Field Workbench</span>
          </button>
          ...
          >
            ...
            <span className="text-xs">Vessel Engine</span>
          </button>
          ...
          >
            ...
            <span className="text-xs">Rail Medium</span>
          </button>
      ...
      <div className="flex-1 w-full max-w-[1720px] mx-auto p-3 sm:p-4 flex flex-col gap-4">
        ...
        {activeTab === 'orbital_energy' && (
          <div className="flex-1">
            <OrbitalCorridorEnergySandbox />
          </div>
        )}

        {/* VIEW: SIMON INFERENCE & EPISTEMIC REGISTRY WORKBENCH (G_t → Ψ_t) */}
        {activeTab === 'simon_registry' && (
          <div className="flex-1">
            <SimonInferenceRegistryWorkbench />
          </div>
        )}

        {/* VIEW: CRUCIBLE DEFENSE AUDITOR (DIRTY DOZEN) */}
        {activeTab === 'crucible' && (
          <div className="flex-1">
            <CrucibleDefenseAuditor />
          </div>
        )}

        {/* VIEW: 12-STAGE CAUSAL WORKFLOW RUNTIME */}
        {activeTab === 'causal_workflow' && (
          <div className="flex-1">
            <CausalWorkflowEngine />
          </div>
        )}

        {/* VIEW: PARALLEL COGNITION ENGINE (5 SPECIALISTS & CONVERGENCE) */}
        {activeTab === 'parallel_cognition' && (
          <div className="flex-1">
        ...
        {/* VIEW: LEARNING HELIX KERNEL (DOUBLE-HELIX PEDAGOGY) */}
        {activeTab === 'learning_helix' && (
          <div className="flex-1">
            <LearningHelixKernel />
          </div>
        ...
        {/* VIEW: DOCTRINE CANON (8 FIELDS, GEOMETRY & VOCABULARY) */}
        {activeTab === 'doctrine' && (
          <div className="flex-1">
            <DoctrineSpace />
          </div>
        ...
        {activeTab === 'ccv01_vehicle' && (
          <div className="flex-1">
            <CCV01VehicleStudio
              ...
              onNavigateToPathfinder={() => setActiveTab('pathfinder_cortex')}
            />
          </div>
        )}

        {/* VIEW 0.5: PATHFINDER COGNITION SUPERVISORY CORTEX */}
        {activeTab === 'pathfinder_cortex' && (
          <div className="flex-1">
            <PathfinderCognitiveLayer
              telemetry={sharedCcvTelemetry}
              commands={ccvPilotCommands}
              onCommitTrajectoryToVehicle={handleCommitTrajectoryToVehicle}
            />
          </div>
        )}
